...
.[!IMPORTANT]: Esta Funcionalidad está presente en todas las paginas por lo que no es necesario crear una nueva (pagina)..
...
El uso de `express` para leer JSON se puede ver y probar en la pagina [Reserva](https://imgur.com/a/r8L6Mfd) en donde se utiliza un fichero JSON llamado `eventos.json` el cual llama a las reservas que tiene un usuario y se muestra en pantalla (.[!IMPORTANT] siempre y cuando el dia que tenga esa reserva este seleccionado y este contenga reservas.).
